...
def clean_data(df):
    ...
    converting category values to binary (0 or 1), removing rows with invalid values (2),
    ...
    # Drop rows with any invalid category values (i.e., 2)
    ...
    df = df.loc[categories.index]  # Align df with filtered categories
    ...
    assert len(df[df.duplicated()]) == 0, "Duplicates were found in the dataframe after cleaning."
    
    # Assert that all category values are binary (0 or 1)
    assert df.iloc[:, 4:].apply(lambda x: x.isin([0, 1]).all()).all(), "Non-binary values found in categories."
